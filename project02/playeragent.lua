...
function CMD.selectHourse( fd ,hourseid)--选择房间进入
	local judge=true--skynet.call("SIMPLEDB","lua","get",hourseid)
	if judge then
		skynet.call("HOURSESERVER","lua","selectHourse",uuid,hourseid)
		playerstatus.myhourseid=hourseid
	else

	end
end
function CMD.createhourse( fd )--创建房间
	--local hourseid=skynet.service("hourseagent")
	local hourseid=skynet.call("HOURSESERVER","lua","createhourse",uuid)
	playerstatus.myhourseid=hourseid
end

function CMD.fastinnerHourse(fd )--快速进入房间
	local hourseid=skynet.call("HOURSESERVER","lua","fastinnerHourse",uuid)
	playerstatus.myhourseid=hourseid
end
...
function CMD.close( fd )--关闭链接，关闭链接前先请求房间看用户是否还在房间（直接关闭手机或者断线的情况）
...
function CMD.leavehourse( fd )--离开房间
	skynet.call("HOURSESERVER","lua","leavehourse",fd, playerstatus.myhourseid)
