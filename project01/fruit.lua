...
function fruit:getEarnMost(  )
	earnmost.maxmoney=math.modf(earnmost.maxmoney)
	return earnmost
end
function fruit:initEarnMost(  )
	earnmost.maxmoney=0
	earnmost.name=""
	earnmost.isdealer=false
end
local systemsearn = {
	money=0
}
function fruit:getSystemsEarn(  )
	return systemsearn.money
end
function fruit:initSystemsEarn(  )
	systemsearn.money=0
end
function fruit:settleAccounts(  )--结算这一局的结果
		self:initEarnMost(  )
		self:initSystemsEarn(  )
		...
		for k,v in pairs(self.player) do--遍历玩家table,把不是中奖的钱全部收为自己所有
				...
				if i~="money" and i~="win" and i~="currentwin" and i~="mylistnum"  then
					if i~= self.resulttable.result then
						if dealer then
							...
							systemsearn.money=systemsearn.money+j
						end
						     --记录玩家当前局所得,负就是失去
						...
						--庄家需要知道他的分数
							...
							systemsearn.money=systemsearn.money - beilu[i]*j
						end
			...
			if v.currentwin>earnmost.maxmoney then
				earnmost.maxmoney=v.currentwin
				earnmost.name=k
			end
			v.win=v.win+v.currentwin
			
		end
		if self.status.score>earnmost.maxmoney then
			earnmost.isdealer=true
			earnmost.maxmoney=self.status.score
			earnmost.name=self:getDealerUid()
		end
