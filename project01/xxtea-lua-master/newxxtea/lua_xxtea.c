...
static int encrypt(lua_State *L) {
	...
	xxtea_long out_len;
	data =(unsigned char *) luaL_checklstring(L, 1, &data_len);
	key  = (unsigned char *)luaL_checklstring(L, 2, &key_len);
	// printf("%s ,%s ,%ld, %ld ,%ld ",data,key,strlen(data),data_len,key_len );

	result =xxtea_encrypt(data, data_len, key, key_len, &out_len);
...
static int decrypt(lua_State *L) {
	...
	key  = (unsigned char *)luaL_checklstring(L, 2, &key_len);
	// printf("%s ,%s ,%ld, %ld ,%ld ",data,key,strlen(data),data_len,key_len );
	result = xxtea_decrypt(data, data_len, key, key_len, &out_len);
	lua_pushlstring(L, (const char *)result, out_len);
	free(result);
